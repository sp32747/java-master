...
public class StreamTest {
	...
	public static void main(String[] args) {
		...
		employeesList.stream().map(employee -> employee.getEmail()).collect(Collectors.toList())
				.forEach(System.out::println);

		// flattening the stream
		employeesList.stream().flatMap(employee -> employee.getPhoneNumbers().stream()).collect(Collectors.toList())
				.forEach(System.out::println);
		...
		Comparator<Employee> SalComp = Comparator.comparing(Employee::getSalary);
